...
export default {
  data() {
    return {
      // 树
      departmentList: [],
      defaultProps: { children: 'subAllotGroup', label: 'allotGroupName' },
      idArr: [],
      filterText: '',
    }
  },
  ...
  methods: {
    ...
    requestTreeData(data) {
      API.allotgroupTree(data).then((res) => {
    ...
    init() {
      if (this.departmentList) {
        ...
        this.departmentList.forEach((m) => {
          this.idArr.push(m.allotGroupId)
        })
      }
    },
    // 搜索过滤
    filterNode(value, data) {
      ...
      return data.allotGroupName.indexOf(value) !== -1
    },
