<template>
	<div class="menu-send-message components-container">
		<div class="send-title-wrap">
			<div class="send-msg-title">直接发送消息</div>
		...
		<el-form ref="createItemForm" :rules="rules" :model="createItem" label-suffix=":">
			...
			<el-form-item :show-message="false" style="width:25%;display:inline-block;" label="所属系统" label-width="80px" prop="systemType">
				<el-select :disabled="createItem.noticeType === 'MSG_UPGRADE'" style="width:100%;" size="mini" v-model="createItem.systemType">
					<el-option v-for="(o, key) in dictMap['systemType']" :key="key" :label="o.label" :value="o.value"> </el-option>
				</el-select>
			</el-form-item>
			<el-form-item :show-message="false" style="width:25%;display:inline-block;" label="通知部门选择" label-width="100px" prop="deptCodes">
				<el-cascader
			...
			<!-- <el-form-item style="width:25%;display:inline-block;" label="通知人选择" label-width="90px" prop="noticeEmployeeId">
				<el-select filterable remote  size="mini" v-model="createItem.noticeEmployeeId" placeholder="请选择员工" :remote-method="getEmployeeList">
					<el-option v-for="(o, key) in employeeOptions" :key="key" :label="o.cname + '  (' + o.employeeNo + ')'" :value="o.employeeId"> </el-option>
				</el-select>
			</el-form-item> -->
			<el-form-item style="width:25%;display:inline-block;" label="优先级" label-width="63px" prop="noticePriority">
...
<script>
import { noticeWebsiteSend } from '@/api/msg'
	...
	methods: {
		...
		submitForm() {
			this.$refs.createItemForm.validate(valid => {
				if (valid) {
					noticeWebsiteSend(this.createItem).then(res => {
