...
<div class="menu">
    ...
    <div class="filter-containe search-layout">
      <div class="filter">
        <el-button size="mini"  @click="handleAdd()" type="primary">
          <i class="el-icon-plus"></i> 菜单权限配置
        </el-button>
      </div>
    </div>
    <el-table v-loading="loading" :data="roleMenuRelativeList"  style="margin:16px 0px">
  ...
  <roleMenuRelative v-if="roleMenuRelativeVisable" ref="roleMenuRelative" @refreshDataList="getList"></roleMenuRelative>
</div>
</template>
<script>
import {roleSelectList,roleMenuRelativeList} from '@/api/permission'
import { mapGetters } from 'vuex'
import Pagination from '@/components/Base/Table/pagination'
import RoleMenuRelative from './roleMenuRelative'
export default {
  name:"roleMenu",
  data() {
    return {
  ...
  methods: {
    ...
    getList(isSearch) {
      ...
      roleMenuRelativeList(this.tableQuery).then(res=>{
        let {totalCount} = res.data
        this.tableConfig.pagination.total = totalCount;
        this.roleMenuRelativeList = res.data.list
        this.loading = false
      })
    },
    handleAdd(row){
      this.roleMenuRelativeVisable=true
      this.$nextTick(() => {
        this.$refs.roleMenuRelative.init(row);
      });
