...
<div class="menu">
    ...
    <div class="filter-containe search-layout">
      <div class="filter">
        <el-button size="small"  @click="handleAdd()" type="primary">
          <i class="el-icon-plus"></i> 规则权限配置
        </el-button>
      </div>
    </div>
    <el-table v-loading="loading" :data="roleRuleRelativeList"  style="margin:16px 0px">
  ...
  <roleRuleRelative v-if="roleRuleRelativeVisable" ref="roleRuleRelative" @refreshDataList="getList"></roleRuleRelative>
</div>
</template>
<script>
import {roleSelectList,roleRuleRelativeList,roleDpGroupDelete} from '@/api/permission'
import { mapGetters } from 'vuex'
import Pagination from '@/components/Base/Table/pagination'
import roleRuleRelative from './roleRuleRelative'
export default {
  name:"roleMenu",
  data() {
    return {
  ...
  methods: {
   ...
   //删除列表
    handleDelete(row){
      this.$confirm('是否确认删除角色名为"' + row.roleName + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        lockScroll: "true",
        type: "warning"
      }).then(function() {
         return roleDpGroupDelete({sysRoleId: row.sysRoleId});
      }).then(() => {
        this.handleSearch();
    ...
    getList(isSearch) {
      ...
      roleRuleRelativeList(this.tableQuery).then(res=>{
        let {totalCount} = res.data
        this.tableConfig.pagination.total = totalCount;
        this.roleRuleRelativeList = res.data.list
        this.loading = false
      })
    },
    handleAdd(row){
      this.roleRuleRelativeVisable=true
      this.$nextTick(() => {
        this.$refs.roleRuleRelative.init(row);
      });
