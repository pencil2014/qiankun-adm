	...
	<div class="operate-group">
		<template>
			<el-button
				...
				:id="item.id ? item.id + scope.$rowIndex : ''"
...
export default {
	...
	methods: {
		...
		// 操作回调 {操作}
		handleOperate({ $rowIndex, row }, item, { callback = () => {} }, $event) {
			let { action } = item
			callback(action, $rowIndex, row, item, $event)
		},
		...
		handleOperateDrop(params) {
			...
			let { action } = item
			callback(action, scope.$rowIndex, scope.row, item)
		},
